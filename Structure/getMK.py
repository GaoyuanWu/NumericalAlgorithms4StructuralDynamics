...
# C -> Connectivity matrix between joints & elements; 2-D array; ne(number of elements) x nj; -1 for node i, 1 for node j, 0 for others
...
def getMK_FEM(joint_coord,C,ele_m,ele_E,ele_A,ele_I):
	jo_num = joint_coord.shape[0] #Number of joints
	ele_num = C.shape[0] #Number of elements  	
	...
	ele_u = C @ joint_coord[:,0] # ele X-difference
	ele_v = C @ joint_coord[:,1] # ele Y-difference
	ele_l = np.sqrt(ele_u**2 + ele_v**2) # ele length
	ele_uni = (np.vstack([ele_u,ele_v]).T) / ele_l # ele unit vector
